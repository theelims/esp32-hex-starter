...
This module is the canonical source of truth. It is:

* **Loaded at scaffold time** by ``extensions/esp32_helpers.py`` (a thin
  loader stub at the repo root) so Copier's Jinja templates can use the
  ``parse_part_number`` filter and ``partition_layout`` global.
* **Available at runtime** in every scaffolded project at
  ``tools/esp32_helpers.py`` so project-local scripts can re-derive the
  same fields if ``hardware/board.yaml`` changes (e.g. swap modules and
  regenerate ``boards/<project>.json`` without re-scaffolding).

Keep this file self-contained — no imports from the rest of the project,
no Jinja2 dependency. The Jinja Extension class lives in the loader stub.
"""
...
def partition_layout(scheme: str, flash_size_mb: int) -> list[dict[str, Any]] | None:
    """Return the partition table rows for ``scheme`` × ``flash_size_mb``.
